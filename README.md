...
1. Explain the difference between `RDBMS` and `SQL`.
  ...
  RDBMS is a relational database management system. This manages the data in the database and can provide more functionality the SQL which is the language for communication with the RDBMS. 
1. Why do tables need a `primary key`?

Tables need a primary to key to distinguish each record in the table. Also primary keys from one table can be use as foreign on another to relate data. 
1. What is the name given to a table column that references the primary key on another table.

A foreign key
1. What do we need in order to have a _many to many_ relationship between two tables.

There must be an intermediate table containing columns that include their primary keys. 
